...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
                 # 3.1 IMPORTAÇÃO DADOS API DISNEY
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
def sync_api_disney():
    ...
    try:
        ...
        logger.exception(f"Erro ao sincronizar os dados da API Disney para a tabela {nome_tabela}: {e}")
        raise


#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
                 # 3.2 IMPORTAÇÃO DADOS API STAR WARS
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
def sync_api_starwars():
    ...
    tabelas = [
        ...
        ("tb_espaco_naves_api_starwars", SyncDataBaseEspacoNavesStarWars(), "armazenamento_espaco_espaco_naves_starwars_db"),
    ...
    try:
        ...
        logger.exception(f"Erro ao sincronizar os dados da API Star Wars: {e}")
        raise


#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
                 # 3.2 IMPORTAÇÃO DADOS API RICK & MORTY
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
def sincronizacao_dados_personagens_db():
    ...
    try:
        ...
        logger.exception(f"Erro ao sincronizar os dados da API para a tabela {nome_tabela}: {e}")
        raise
...
def execucao_aplicacao():
    ...
    try:
        ...
        logger.exception(f"Ocorreu um erro durante a execução do processo de sincronização de dados: {e}")
